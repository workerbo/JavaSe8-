...
- 线程状态的基本操作【线程间一种通信方式】
  ...
  3. 一种是当线程处于阻塞状态或者试图执行一个阻塞操作时，我们可以使用实例方法interrupt()进行线程中断，执行中断操作后将会抛出interruptException异常(该异常必须捕捉无法向外抛出)并将中断状态复位；

  4.  另外一种是当线程处于运行状态时，我们也可调用实例方法interrupt()进行线程中断，但同时必须手动判断中断状态，并编写中断线程的代码(其实就是结束run方法体的代码)。有时我们在编码时可能需要兼顾以上两种情况，那么就可以如下编写：
  ...
  5. join方法可以看做是线程间协作的一种方式，很多时候，一个线程的输入可能非常依赖于另一个线程的输出。

  6. sleep()方法只是会让出CPU并不会释放掉对象锁；wait()方法必须要在同步方法或者同步块中调用，也就是必须已经获得对象锁。会释放掉对象锁。

  7. yield()方法只允许与当前线程具有相同优先级的线程能够获得释放出来的CPU时间片。调用yield方法并不会让线程进入阻塞状态，而是让线程重回就绪状态。不能控制具体的交出CPU的时间。

  8. 阻塞【获取锁失败】和等待区别【等待一个条件满足】
...
`synchronized` 使用对象作为锁，每个对象都关联一个 Monitor (监视器，又叫管程)。Monitor 对象的地址保存在对象头的 Mark Word 中，它有三个重要的属性：

Owner: 记录持有该锁的线程

EntryList: 阻塞队列，当线程进入同步代码时，如果尝试获取对象锁失败，会转化为阻塞状态并进入该队列。**在对象锁释放后，阻塞队列中的线程会重新开始竞争锁。**

WaitSet: 等待池，当线程调用对象的 `wait()` 方法时，会释放持有的锁，然后转化为等待状态并进入等待池。**等待池中的线程不会去竞争锁；其他线程调用 `notify()`或 `notifyAll()` 会唤醒等待池中的线程。线程被唤醒后会转移到阻塞队列中，重新竞争锁。**

**在 wait 的范式中，我使用了 while 循环。原因是: notify 只能保证在通知的时间是条件是满足的，而被唤醒线程恢复执行的时间点与通知的时间点之间，往往存在着一定的延迟。所以当它执行时，条件可能已经变成不满足了。**

结束一个线程最直接的方式就是调用 `Thread` 对象的 `stop()` 方法，但该方法存在着安全隐患：

`stop()` 方法会直接终止线程，并立即释放线程所持有的锁。**锁的一个重要最要是维护数据的一致性，如果在同步代码中，线程未执行完成就释放锁，会导致数据不一致。**
...
- wait 是 Object 类的实例方法，sleep 是 Thread 类的静态方法
- wait 必须配合 `synchronized` 关键字使用，sleep 可以直接调用
- wait 会释放线程持有的锁，sleep 不会释放锁
...
- JMM核心为两个部分：[happens-before规则以及内存抽象模型](https://771592594.github.io/xiaoyue/2021/10/24/Java-%E5%86%85%E5%AD%98%E6%A8%A1%E5%9E%8B%E5%A6%82%E4%BD%95%E8%A7%A3%E5%86%B3%E5%8F%AF%E8%A7%81%E6%80%A7%E5%92%8C%E6%9C%89%E5%BA%8F%E6%80%A7%E9%97%AE%E9%A2%98/)
...
- 在Java中，AtomicStampedReference<E>也实现了这个作用，它通过包装[E,Integer]的元组来对对象标记版本戳stamp，从而避免ABA问题
  ...
  在 HotSpot 虚拟机里，对象在堆内存的存储空间可以划分为三个部分：

  - 对象头（header）
- 实例数据（Instance Data）
  - 对齐填充（Padding）
  
  对象头由 **保存对象自身运行时数据的 Mark Word** 和 **对象的类型指针** 组成
...
七：[锁粒度](https://771592594.github.io/xiaoyue/2021/10/11/Java-%E9%94%81%E7%B2%92%E5%BA%A6%E4%B8%8E%E6%AD%BB%E9%94%81%E9%97%AE%E9%A2%98/)

产生死锁需要四个必要条件：

1. 互斥，共享资源 X 和 Y 只能被一个线程占用；
2. 占有且等待，线程 T1 已经取得共享资源 X，在等待共享资源 Y 的时候，不释放共享资源 X；【锁管理器，一次获取所有锁】
3. 不可抢占，其他线程不能强行抢占线程 T1 占有的资源；【借助ReentrantLock限定时间】
4. 循环等待，线程 T1 等待线程 T2 占有的资源，线程 T2 等待线程 T1 占有的资源。【对锁排序，从小到大】



1. 在编写并发代码时，锁的粒度越小，程序的运行效率越高。在保证正确性的情况下，应该尽可能使用细粒度锁。
2. 如果存在多个需要保护的资源，只使用其中一个资源作为锁是不够的，锁必须覆盖所有需要保护的资源。
3. 使用细粒度锁保护多个资源时，如果实现不当，可能会出现死锁问题。
4. 对于死锁问题，我们要了解它产生的四大必要条件。只要能够破坏其中一条，就能避免死锁的产生。



**可运行状态**：线程可以被分配CPU执行。在这个状态下，线程在操作系统中被真正地创建了。

**运行状态**：当线程被操作系统分配CPU时，就会进入运行状态。在运行状态下，线程会执行程序中的语句。当CPU的时间片用完后，线程就会变回可运行状态。
