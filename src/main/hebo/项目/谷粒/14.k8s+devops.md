...
![1641644064476](/1641644064476.png)
...
三个时代：
	传统部署时代：缺点（1.资源不隔离；2.扩展性差）
	虚拟化部署时代：缺点（1.资源浪费，多个虚拟操作系统）
	docker容器部署时代：所有App运行在运行时容器环境内，无需虚拟机就可以做到资源隔离
		优点：资源隔离性（内存性放在一起，升级内存）、扩展性、适合分布式大量部署

kubernetes的作用：就是管理容器（docker）
```

### 服务发现和负载均衡

```
kubernetes的负载均衡是面对任何服务的（针对所有类型的容器，App不仅仅是java）
```

### 存储编排

```
kubernetes允许挂载任意存储系统，本地存储、公共云提供商等
...
![1641644957799](/1641644957799.png)
...
![1641644966336](/1641644966336.png)
...
![1641644982410](/1641644982410.png)



## 2.组件

![1641645055519](/1641645055519.png)

**<u>Master与Node关系：</u>**（master暴露Api，由Master调度Node）

![1641645163030](/1641645163030.png)

### Master组件

```
Master不包含Docker、kubelet
...
![1641645284519](/1641645284519.png)
...
![1641645328080](/1641645328080.png)
...
![1641645360946](/1641645360946.png)
...
![1641645397353](/1641645397353.png)

#### kube-controller-manager

```
接受scheduler的调度，实现真正的执行调度
```

![1641645533006](/1641645533006.png)

![1641646458148](/1641646458148.png)

### Node组件

**<u>Node节点架构：</u>**

![1641645690814](/1641645690814.png)

```
Node包含了 Docker、kubelet
```
...
```
功能：
	将多个container组合后创建pod
	销毁pod
```

![1641645737493](/1641645737493.png)
...
功能：
	相当于网卡、路由器，维护pod的IP
	接受网络请求进行解析并将请求转发到指定pod
```

![1641645744815](/1641645744815.png)

#### Docker

![1641645778735](/1641645778735.png)
...
![1641646058264](/1641646058264.png)
...
![1641646158028](/1641646158028.png)



![1641646173753](/1641646173753.png)

![1641646440429](/1641646440429.png)

![1641646451766](/1641646451766.png)

![1641646509717](/1641646509717.png)

![1641646516571](/1641646516571.png)

![1641646776062](/1641646776062.png)
...
Auth+Api：认证 接收 API，将调度请求存储到etcd
...
```
1.通过kubectl提交一个创建副本的请求，通过ApiServer写入etcd中
2.Controller Manager通过ApiServer监听资源变化的接口监听到此RC事件
3.Controller Manager分析集群中是否有对应的Pod实例，如果没有则根据Pod模板创建Pod对象存储到etcd中
4.secheduler监听到事件，执行一个复杂调度，为Pod选定一个落户的Node，然后将此Node信息写入到etcd
5.目标Node上的kubelet进程通过ApiServer监听到新生Pod，在该Node上创建一个容器启动Pod
```

![1641647402848](/1641647402848.png)

![1641647423282](/1641647423282.png)



![1641647324089](/1641647324089.png)

![1641646985191](/1641646985191.png)
...
![1641647187934](/1641647187934.png)
...
![1641647986880](/1641647986880.png)

## 1.k8s集群安装

### 1.kubeadm

![1641648045268](/1641648045268.png)
