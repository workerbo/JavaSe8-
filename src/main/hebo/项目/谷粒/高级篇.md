...
2.一个nginx.conf【总】会包含多个server快（conf.d内*.conf文件）【分】
...
4.nginx反向代理时会丢失host信息，配置server块，转发时同时转发hosts【网关可能会根据Host做转发】
    server {
        listen 80;
        server_name gulimall.com;
...
public class StockReleaseListener {
    ...
    /** QWVEPFOPG[RGIDFLJGVLDJKASLFHDSKLAFJLJFDKLJFLASDJKLF]
     * 客户取消订单，监听到消息
     */
    @RabbitHandler
...
* 消息丢失:
  ...
  * 解决：消息确认处：发送消息时同时将消息持久化到MQ中并插入DB（DB消息状态为已抵达）
             生产者：当出现异常时在catch处修改消息状态为错误抵达
